...
start() {
  cd "$ROOT"
  HF_HOME=${HF_HOME:-/data/lychee_ja/hf_home} JEV_STATE_DIR="$STATE" CUDA_VISIBLE_DEVICES="$GPU" nohup "$ROOT/.venv/bin/python" -m openvons.voice.demo_server --app "${JEV_APP:-examples.road_cameras.app}" --port "$PORT" --tts "$TTS_URL" ${JEV_SSL_DIR:+--ssl-dir "$JEV_SSL_DIR"} >"$LOG" 2>&1 &
  echo $! > "$PID"
