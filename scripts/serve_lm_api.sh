...
start() { cd "$ROOT"; DM_LLM_URL="${DM_LLM_URL:-http://127.0.0.1:8300/v1}" DM_PORT="$PORT" HF_HOME="${HF_HOME:-/data/decision_model/hf_home}" nohup "$ROOT/.venv/bin/python" -m openvons.lm.api.server >"$LOG" 2>&1 & echo $! >"$PID"
