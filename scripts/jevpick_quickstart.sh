#!/usr/bin/env bash
# JevPick を 1 GPU で一通り動かす (Tool Call、既定 Qwen3-4B を 4-bit NF4 で)。所要 20〜40 分、ディスク ~10GB (N=300 のとき)。
# 27B を 4-bit で: JEVPICK_MODEL=Qwen/Qwen3.8-27B JEVPICK_DRAFT=incoai/Qwen3.8-27B-DFlash2 scripts/jevpick_quickstart.sh 0   (VRAM ~24GB)
...
QUANT="${JEVPICK_QUANT:-nf4}"; Q=(); [ "$QUANT" != "none" ] && Q=(--quant "$QUANT") && TAG="$TAG-$QUANT"   # 既定は 4-bit (bitsandbytes NF4)。bf16 は JEVPICK_QUANT=none
echo "data dir: $D   model: $MODEL ($QUANT)   draft: $DRAFT"
...
$PY openvons/jevpick/data/collect.py --model "$MODEL" --no-think --domain toolcall --prompts "$D/prompts_v2.jsonl" --out "$D/traces_v2_$TAG.jsonl" --batch 24 --max-new 128 "${Q[@]}"
echo "[3/6] hidden state + DFlash 候補"
$PY openvons/jevpick/data/extract.py --model "$MODEL" --draft "$DRAFT" --traces "$D/traces_v2_$TAG.jsonl" --out "$D/extract_v2_${TAG}_0" "${Q[@]}"
...
$PY experiments/jevpick/phase4_runtime/runtime_v2.py --model "$MODEL" --draft "$DRAFT" --prompts "$D/prompts_v2.jsonl" --traces "$D/traces_v2_$TAG.jsonl" \
   --domain toolcall --n 20 --L 8 --modes baseline,prior,scorer,dflash,union,hybrid --scorer "$D/jevpick_toolcall_union_L8.pt" --out "experiments/jevpick/phase4_runtime/runtime_quickstart_$TAG.json" "${Q[@]}"
