...
function render(d) {
  ...
  d.summary.forEach(it => {
    ...
    s.windows.forEach(w => { const i = document.createElement('i'); const p = w.p[0]; i.style.background = `rgba(248,81,73,${(0.08 + p * 0.92).toFixed(2)})`; i.title = `${w.t0.toFixed(1)}〜${w.t1.toFixed(1)} 秒: はい ${(p * 100).toFixed(0)}% / いいえ ${(w.p[1] * 100).toFixed(0)}% / 判別できない ${(w.p[2] * 100).toFixed(0)}%`;
    ...
    const pm = document.createElement('div'); pm.className = 'pmax'; pm.textContent = (it.p_max * 100).toFixed(0) + '%'; row.appendChild(pm);
    tl.appendChild(row);
  });
